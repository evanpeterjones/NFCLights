...
public class light {
     ...
     */
    public light(String ip, int port) {
        this.ip = ip;
        this.port = port;
    }
     ...
     */
    public boolean switchOn() throws IOException, JSONException {
        String jsonData = sendCommand(COMMAND_SWITCH_ON);
        if(jsonData.length() > 0) {
            JSONArray test = new JSONArray(jsonData);
            JSONObject jo = test.getJSONObject(0);
            int errorCode = Integer.parseInt(jo.getString("err_code"));
            return errorCode == 0;
        }
        return false;
    }
     ...
     * @return true if successful
     */
    public boolean switchOff() throws IOException, JSONException {
     ...
     */
    public boolean isOn() throws IOException, JSONException {
        String jsonData = sendCommand(COMMAND_INFO);
        if(jsonData.length() > 0) {
            JSONArray test = new JSONArray(jsonData);
            JSONObject jo = test.getJSONObject(0);
            int state = Integer.parseInt(jo.getString("relay_state"));
